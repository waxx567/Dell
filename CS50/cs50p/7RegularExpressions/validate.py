...
# Output either or after checking address
...
# But back to
''' print("Valid") if re.search(r"^\w+@(\w+\.)?\w+\.edu$", email, re.IGNORECASE) else print("Invalid")'''
# If the email address is david.malan@cs50.harvard.edu, add a | (or) and a literal .
# Bracket the thought which could also be expressed as [a-z0-9_\.] so
''' print("Valid") if re.search(r"^(\w+|\.)@(\w+\.)?\w+\.edu$", email, re.IGNORECASE) else print("Invalid")'''
# Or
''' print("Valid") if re.search(r"^[a-z0-9_\.]@(\w+\.)?[a-z0-9_]\.edu$", email, re.IGNORECASE) else print("Invalid")'''
# As above, so below